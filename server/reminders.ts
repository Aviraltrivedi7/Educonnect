...
import { cleanupExpiredComparisonSharingAuditExports, cleanupExpiredTrendExportDownloads, deliverScheduledLearningReminder, deliverScheduledMonthlyCertificateAuditReport, deliverScheduledMonthlyComparisonReview } from "./db";
...
export async function handleScheduledComparisonSharingExportRetention(req: Request, res: Response) {
  try {
    const user = await sdk.authenticateRequest(req);
    if (!user.isCron || !user.taskUid) return res.status(403).json({ error: "cron-only" });
    return res.json(await cleanupExpiredComparisonSharingAuditExports(user.taskUid));
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error("[Comparison sharing export retention] scheduled cleanup failed", error);
    return res.status(500).json({ error: message, context: { url: req.originalUrl }, timestamp: new Date().toISOString() });
  }
}
